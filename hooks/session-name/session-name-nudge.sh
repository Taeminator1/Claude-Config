...
# A re-nudge flag is dropped by the SessionStart hook on /clear. /clear is an
# explicit "rename this conversation" signal, so we ALWAYS ask via choices on the
# next prompt -- no ai-title auto-apply, no grace. (Distinct from a fresh session,
# which auto-names from ai-title silently.) Consume the flag and grace marker, and
# stamp nudged so we ask exactly once.
if [ -f "$RENUDGE_FILE" ]; then
  rm -f "$RENUDGE_FILE" "$GRACE_FILE" 2>/dev/null || true
  mkdir -p "$HOME/.claude/.session-nudged" 2>/dev/null || true
  : > "$NUDGED_FILE" 2>/dev/null || true

  CONTEXT="방금 /clear로 대화가 초기화되었습니다. 이전 세션 이름이 그대로 남아 있을 수 있으니, 이번 대화에 맞는 이름을 새로 정합니다. 사용자의 이번 요청에 대한 답변을 만들기 전에, 먼저 AskUserQuestion 도구로 이 세션의 이름을 짧게 무엇으로 할지 사용자에게 존댓말로 물어보세요. 선택지는 다음 순서로 구성하세요: 1번 '현재 이름을 그대로 유지', 그다음 사용자의 이번 요청 내용을 기반으로 추천 세션 이름 1~2개, 마지막에 '아무 작업도 하지 않기'를 넣으세요. 사용자가 추천 이름이나 직접 입력한 이름을 주면 다음을 실행해 기록하세요: bash ~/.claude/hooks/session-name/set-session-name.sh ${SESSION_ID} \"<이름>\". (session_id=${SESSION_ID}) 사용자가 '현재 이름을 그대로 유지'나 '아무 작업도 하지 않기'를 선택하면 이름을 기록하지 말고 곧바로 원래 요청을 처리하세요. 셋 중 하나를 처리한 뒤 원래 요청을 처리하세요."
...
# An ai-title already exists -> apply it now and show it. Covers resume, or a
# title generated before any Stop hook fired this run.
...
# No ai-title yet. Give it a grace turn: the Stop hook after this response names
# the session silently. Only on a *later* prompt that STILL has no ai-title do we
...
# pick a name. Stamp nudged so we ask only once.
mkdir -p "$HOME/.claude/.session-nudged" 2>/dev/null || true
: > "$NUDGED_FILE" 2>/dev/null || true
